...
// CELCAT API helpers
// Note: needs an active CELCAT browser session to work (client-side only). Idk what we're gonna do here...client side session is easiee but server side session is cleaner 
//TO DO ---> after the girls test see if i need to change how the auth is managed, if theres errors its porbably that 
// TO DO ----> Invalid roomIds return empty results instead of errors - we validate against rooms.json 
